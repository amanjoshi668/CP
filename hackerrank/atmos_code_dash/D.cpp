...
int main(int argc, char*argv[]){
    ...
    lo n,m,k,q;
    cin>>n>>m>>k>>q;
    lo dp_down[n+1][n+1],dp_up[n+2][n+2];
    memset(dp_down,0,sizeof(dp_down));
    memset(dp_up,0,sizeof(dp_up));
    REP(0,k){
        lo x,y;
        cin>>x>>y;
        dp_down[x][y]=dp_up[x][y]=-1;
    }
    if(dp_down[1][1]!=-1)dp_down[1][1]=1;
    REPE(1,n+1,1,n+1){
        if(dp_down[i][j]==-1)continue;
        if(dp_down[i-1][j]!=-1)dp_down[i][j]+=dp_down[i-1][j];
        if(dp_down[i][j-1]!=-1)dp_down[i][j]+=dp_down[i][j-1];
        dp_down[i][j]%=MOD;
    }
    if(dp_up[n][n]!=-1)dp_up[n][n]=1;
    IREP(n,1)IREPV(j,n,1){
        if(dp_up[i][j]==-1)continue;
        if(dp_up[i+1][j]!=-1)dp_up[i][j]+=dp_up[i+1][j];
        if(dp_up[i][j+1]!=-1)dp_up[i][j]+=dp_up[i][j+1];
        dp_up[i][j]%=MOD;
    }
    while(q--){
        lo x,y;
        cin>>x>>y;
        if(dp_down[x][y]==-1)cout<<"0"<<endl;
        else cout<<(dp_down[x][y]*dp_up[x][y])%MOD<<endl;
    }
    print_matrix(dp_down,n+1,n+1);
    print_matrix(dp_up,n+1,n+1);
    return 0;
}
