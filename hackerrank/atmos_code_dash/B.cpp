...
vl g[INF],visited(INF,-1);
lo ans=LLONG_MAX;
void dfs(lo node, lo par){
    //debug(node);
    visited[node] = LLONG_MAX;
    lo m1,m2;
    m1=m2=LLONG_MAX;
    TRV(g[node])if(visited[it]==-1){
        dfs(it,node);
        if(visited[it]<m1){
            m2 = m1;
            m1 = visited[it];
        }
        else if(visited[it]<m2){
            m2=visited[it];
        }
        visited[node]=min(visited[node],visited[it]);
    }
    if(visited[node]==LLONG_MAX)visited[node]=0;
    else visited[node]++;
    //debug2(m1,m2);
    if(m2!=LLONG_MAX)ans = min(ans, m1+m2+2);
}
