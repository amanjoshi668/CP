...
int main(int argc, char*argv[]){
    ...
    vl inv_fact(n+1,1);
    lo ans = fact[n];
    REP(2,n+1)fact[i]=(fact[i-1]*i)%MOD;
    REP(2,n+1)inv_fact[i]=power(fact[i],MOD-2);
    TRV(a){
        ans = (ans*power(power(it.Y,it.X),MOD-2))%MOD;
        ans = (ans*inv_fact[it.X])%MOD;
    }
    cout<<ans;
    return 0;
}
