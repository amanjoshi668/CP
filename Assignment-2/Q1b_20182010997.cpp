...
int main(int argc, char*argv[]){
    ...
    auto result = suffix_lcp_array(s);
    cout<<"fjdkfhkdsf"<<endl;
    multiset<lo> long_lcp;
    REP(0,k-1)long_lcp.insert(result.Y[i]);
    //derr(long_lcp.size());
    lo res = *long_lcp.begin();
    REP(k-1,s.length()){
        auto rem = long_lcp.find(result.Y[i-k+1]);
        long_lcp.erase(rem);
        long_lcp.insert(result.Y[i]);
        //derr(long_lcp.size());
        res = max(res,*long_lcp.begin());
    }
    cout<<res<<endl;
    return 0;
}
